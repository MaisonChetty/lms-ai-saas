        ...
        {modules.map((module, index) => {
          ...
          return (
            <AccordionItem
              key={module._id}
              value={module._id}
              className="border border-border rounded-xl overflow-hidden bg-card data-[state=open]:bg-secondary/70 transition-colors"
            >
              <AccordionTrigger className="px-5 py-4 hover:no-underline hover:bg-secondary/70 transition-colors">
                  ...
                  {userId && total > 0 && (
                    <div className="hidden sm:flex items-center gap-3 shrink-0 w-36">
                      <Progress
                        ...
                        className="flex-1 h-2 bg-secondary [&>div]:bg-emerald-500"
                      />
                      <div className="w-5">
                        {isModuleComplete && (
                  ...
                  {module.lessons?.map((lesson, lessonIndex) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-center gap-2.5 pl-2 pr-3 py-2 rounded-lg hover:bg-secondary/70 transition-colors group"
                      >
                        {completed ? (
                        ...
                        {hasVideo && (
                          <Play className="w-4 h-4 text-muted-foreground group-hover:text-violet-400 transition-colors" />
                        )}
                      </Link>
                    );
                  })}
